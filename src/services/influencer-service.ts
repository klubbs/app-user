...
export class InfluencerService {
  ...
  static async getAllCouponsByInfluencer(): Promise<CouponAndOffersByInfluencerResponse[]> {
    const { data } = await connectionHandler('KLUBBS_API_URL').get<
      IResponseMessage<CouponAndOffersByInfluencerResponse[]>
    >('influencer/coupons');
  ...
  static async getAllOffers(): Promise<GetAllOffersResponse[]> {
    const { data } = await connectionHandler('KLUBBS_API_URL').get<
      IResponseMessage<GetAllOffersResponse[]>
    >('stores/coupon');
  ...
  static async linkCouponInOffers(masterCoupons: string[], couponId: string): Promise<void> {
    await connectionHandler('KLUBBS_API_URL').post('influencer/coupons/link', {
      master_coupons: masterCoupons,
      coupon_id: couponId,
    });
  }

  static async RemoveOffer(offerId: string, couponId: string) {
    await connectionHandler('KLUBBS_API_URL').put('influencer/coupons/offers/remove', {
      offers: [offerId],
      coupon_id: couponId,
    });
  }
    ...
    if (error.statusCode === 422) {
      ...
      if (actual === 'CODE') {
        NotificationsFlash.customMessage(
          'Código inválido',
          'Espaços são proibidos e necessário 10 caracteres.',
          'NEUTRAL',
        );
        return;
      }
      ...
      if (actual === 'CODE') {
        NotificationsFlash.customMessage(
          'Código já em uso',
          'Este código de cupom já esta sendo usado',
          'WARNING',
        );
        return;
      }
  ...
  static catchLinkCoupon(error: IError) {
    ...
    if (error.statusCode === 412) {
      ...
      if (actual === 'MASTER COUPON') {
        NotificationsFlash.customMessage(
          'Desculpe',
          'Uma das ofertas que você escolheu não esta mais válida',
          'NEUTRAL',
        );
        return;
