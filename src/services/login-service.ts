...
class LoginService {
  static async login(mail: string, password: string): Promise<ILoginResponse> {
    const { data } = await connectionHandlerAuthZN.get<IResponseMessage<ILoginResponse>>(
      'auth/login/user',
      {
        params: {
          mail: mail,
  ...
  static async registerUser(user: IRegisterUser, code: string): Promise<ICreateUserResponse> {
    ...
    const { data } = await connectionHandler('KLUBBS_API_URL').post<
      IResponseMessage<ICreateUserResponse>
    >('users', {
      mail: domain.mail,
      phone: domain.phone,
  ...
  private static registerContract(
    user: IRegisterUser,
    code: string,
  ): IRegisterUser & { code: string } {
  ...
  static async updatePassword(password: string, mail: string, code: string): Promise<void> {
    await connectionHandler('KLUBBS_API_URL').put('users/update/password', {
      password: password,
      mail: mail,
      code: code,
    });
  ...
  static async mailAlreadyInUse(email: string): Promise<boolean> {
    const { data } = await connectionHandler('KLUBBS_API_URL').get<IResponseMessage<boolean>>(
      `users/infos/mail`,
      { params: { mail: email } },
    );
...
class LoginServiceExceptions {
  static catchValidateMail(error: IError) {
    if (error.statusCode === 422) NotificationsFlash.invalidMail();
    else {
